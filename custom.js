...
$( ".menu-btn" ).click(function() {
 ...
 function setWeekDay(val, txt) {
   DateSelected = txt;
   RowNumSelected = val;
 }

 function getDogDataNoParams(event) {
   dayRow.style.display = "none";
   timeRow.style.display = "none";
   weekRow.style.display = "none";
   customerformdiv.style.display = "none";
   
   document.getElementById("mark-customer").style.display = "none";
   ...
   gapi.client.sheets.spreadsheets.values
     ...
     .then(
       function (response) {
         ...
         if (res.length > 0) {
           displayDogTable(res, RowNumSelected);
         } else {
           alert("No Data Found");
         }
       },
                ...
                <ul>
                    ...
                    <li>
                        <label class="label-switch switch-primary">
                            <input type="checkbox" class="switch switch-bootstrap status" name="status" id="status" onclick="yesno(${c.toString()},${val.toString()}) autocomplete="on">
